...
MIN_TURN      = 60
MAX_TURN      = 150
REVALIDATE_EVERY = 20   # Re-check with MobileNet every 20 frames
...
try:
    while True:
        ...
        frame = cv2.flip(frame, 1)   # Restored — prevents mirrored L/R commands
        out.write(frame)
        ...
        if not tracking:
            ...
            if success:
                ...
                # Re-validate every N frames — confirm still tracking a person
                if frame_count % REVALIDATE_EVERY == 0:
                    person_box = detect_person(frame)
                    if person_box is None:
                        print("Re-validation failed — no person found, re-scanning")
                        tracking  = False
                        tracker   = None
                        lost_time = None
                        cx_history.clear()
