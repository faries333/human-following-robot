...
TARGET_HEIGHT = 160   # box height in pixels to stop at — tune this!
HEIGHT_MARGIN = 50    # tolerance around target height
MAX_MOVE      = 0.50  # max normalized distance person can move between frames
DETECT_EVERY  = 3     # run MobileNet every 3rd frame for better performance
...
def find_person_a(persons, last_cx, last_cy):
    """Find detection closest to Person A's last known position."""
...
try:
    while True:
        ...
        # Run MobileNet every DETECT_EVERY frames for better performance
        detect_count += 1
        if detect_count % DETECT_EVERY == 0:
            last_persons = get_all_persons(frame)
        ...
        if not tracking:
            ...
            if match is not None:
                ...
                # Draw red boxes on all other people
                for p in persons:
                    if p is not match:
                        px, py, pw, ph = p['box']
